...
class AuthService {
  ...
  Future<UserModel?> getCurrentUserModel() async {
    try {
      ...
      if (doc.exists) {
        developer.log('User model loaded successfully', name: 'auth_service');        return UserModel.fromFirestore(doc.data() as Map<String, dynamic>, doc.id);
      } else {
    ...
    try {
      ...
      switch (e.code) {
        case 'account-exists-with-different-credential':
          errorMessage = 'Esta cuenta existe con diferente método de login';          break;
  ...
  Future<void> _saveUserToFirestore(User? user) async {
    ...
    try {
      ...
      if (!doc.exists) {
        // Calcular trialEndDate (7 días desde ahora)
